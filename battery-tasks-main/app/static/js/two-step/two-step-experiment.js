...
const task_info = {
  ...
  aliens: jsPsych.randomization.shuffle([1,2,3,4]).map((j,i) => {
    return '../../static/img/two-step/aliens_svg/A'+j+'-'+color_names[mapping[i<2?0:1]]+'.svg'
  })
}
...
const practice_info = {
  ...
  aliens: jsPsych.randomization.shuffle([1,2,3,4]).map((j,i) => {
    return '../../static/img/two-step/aliens_svg/P'+j+'-'+color_names[mapping[i<2?2:3]]+'.svg'
  })
}
